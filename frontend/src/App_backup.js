              ...
              <div key={message.id} className={`message ${message.type}`}>
                ...
                <div className="message-content">
                  {(message.message || '').split('\n').map((line, index) => {
                    ...
                    // Enhanced table parsing for financial data
                    if (line.includes('|') && (line.includes('---') || line.match(/\|.*\|.*\|/))) {
                      // This is a table row - collect all table rows
                      let tableRows = [line];
                      let tableIndex = index + 1;
                      const analysisLines = finalResult && finalResult.analysis ? finalResult.analysis.split('\n') : [];
                      
                      // Collect remaining table rows
                      while (tableIndex < analysisLines.length) {
                        const nextLine = analysisLines[tableIndex];
                            ...
                            <table style={{ 
                              ...
                              <thead>
                                <tr style={{ backgroundColor: 'var(--bg-tertiary)' }}>
                                  {headerRow.split('|').filter(cell => cell.trim()).map((cell, cellIndex) => (
                              ...
                              <tbody>
                                {dataRows.map((row, rowIndex) => (
                                  <tr key={rowIndex} style={{ 
                                    backgroundColor: rowIndex % 2 === 0 ? 'var(--bg-card)' : 'var(--bg-elevated)',
        ...
        {finalResult && (
          <div className="results-section">
            ...
            <div className="result-content professional-report">
              {(() => {
                try {
                  if (finalResult && finalResult.analysis && typeof finalResult.analysis === 'string') {
                    return (
                      <div className="report-text">
                        {finalResult.analysis.split('\n').map((line, index) => {
                          try {
                            ...
                            return <p key={index} className="error-line">Error rendering line: {lineError.message}</p>;
                          }
                        })}
                      </div>
                    );
                  ...
                  return (
                    <div className="report-text">
                      ...
                      <details>
                        ...
                        <pre>Error: {renderError.stack}</pre>
                      </details>
                    </div>
                  );
                }
